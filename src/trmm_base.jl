...
function LeftUpperTRMM!(A, B; n_threads = (16,16))
    ...
    LeftUpperTRMM_kernel!(backend, n_threads)(A, B, ndrange = size(B))
end
...
function RightLowerTRMM!(A, B; n_threads = (16,16))
    backend = get_backend(A)
    # could not use overloading with only 2 args
    RightLowerTRMM_kernel!(backend, n_threads)(A, B, ndrange = size(B))
end
...
function RightUpperTRMM!(A, B; n_threads = (16,16))
    backend = get_backend(A)
    # could not use overloading with only 2 args
    RightUpperTRMM_kernel!(backend, n_threads)(A, B, ndrange = size(B))
end